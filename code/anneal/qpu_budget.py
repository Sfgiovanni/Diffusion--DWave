...
DEFAULT_CAP_SECONDS = 20 * 60.0
DEFAULT_MARGIN = 0.10

# Absolute ceiling on spendable QPU time, independent of whatever `cap_seconds`/`margin` a caller
# passes in. This run must never spend past 20 minutes of real hardware time under any
# circumstances -- so it is enforced here, at the one chokepoint every submission (`charge()`)
# goes through, rather than trusted to callers (a CLI flag, an env var, a script default) that
# could be misconfigured or bypassed. `usable_seconds()` clamps to this no matter what a caller
# requests; raising it requires editing this constant, not passing a bigger number in.
ABSOLUTE_CEILING_SECONDS = 20 * 60.0
...
class QpuBudget:
    ...
    @staticmethod
    def _usable_seconds(state: dict, fallback_cap: float, fallback_margin: float) -> float:
        """`cap * (1 - margin)`, clamped to `ABSOLUTE_CEILING_SECONDS` no matter what the caller
        (or a stale/misconfigured ledger) requests -- see the constant's docstring."""
        cap = float(state.get("cap_seconds", fallback_cap))
        margin = float(state.get("margin", fallback_margin))
        return min(cap * (1.0 - margin), ABSOLUTE_CEILING_SECONDS)

    def remaining_seconds(self) -> float:
        """Budget still spendable, i.e. the cap less the untouchable margin, less what is gone."""
        with self._locked() as handle:
            state = self._read(handle)
        usable = self._usable_seconds(state, self.cap_seconds, self.margin)
    ...
    def charge(self, estimate_us: float, label: str = ""):
        ...
        with self._locked() as handle:
            state = self._read(handle)
            cap = float(state.get("cap_seconds", self.cap_seconds))
            usable_us = self._usable_seconds(state, self.cap_seconds, self.margin) * 1e6
            ...
            if spent_us + estimate_us > usable_us:
                raise BudgetExceeded(
                    ...
                    f"absolute ceiling {ABSOLUTE_CEILING_SECONDS:.0f}s). "
