"""Digital-arm comparison on QuDDPM's (arXiv:2310.05866) "circular states" target family
(`qstate/targets.generate_circular_pool`) -- their Table I "topology" benchmark, `N=1`, a
purely geometric ring of pure states (`e^{-ix*Y}|0>`, `x~U[0,2pi]`), the cheapest and most
structurally different of the two new families added this round (the other,
`generate_correlated_noise_pool`, is `N=2` and noise-driven rather than geometric).

Same arms, forward process, metrics and ablation discipline as `qstate_gate2_supervised.py`
-- only the target distribution changes. Single `T=200`, `encoding='quadrature'` (not the
full T x encoding grid) to keep this addition's scope proportionate; `docs/QSTATE_DIFFUSION
.md` states this reduction plainly, matching this project's standing convention.
"""
...
def cell(seed, draw, device):
    ...
    ds = ROOT + seed
    train_pool, _ = targets.generate_circular_pool(max(N_TRAIN, 100), seed=ds)
    val_pool, _ = targets.generate_circular_pool(max(N_VAL, 100), seed=ds + 1)
    test_pool, _ = targets.generate_circular_pool(max(N_TEST, 100), seed=ds + 2)
    ...
    for name, res in results.items():
        ...
        rows.append(dict(N=N, T=T, target_kind='circular', encoding=ENCODING, data_seed=seed,
                         unitary_draw=draw, method=name, n_train=N_TRAIN, n_val=N_VAL,
                         n_test=N_TEST, fidelity_mean=float(np.mean(m['fidelity'])),
...
def main():
    ...
    device = 'cuda'
    path = OUT / 'gate2_circular_digital.parquet'
    ...
    payload = dict(
        provenance=provenance.provenance(
            script='experiments/qstate_gate2_circular_digital.py',
        ...
        scope_note=("QuDDPM's 'circular states' target family (Table I, Fig. 6), N=1, "
                    "digital arm. Single T/encoding (not the full grid) -- a proportionate "
                    "reduction for this addition, stated plainly per project convention."),
        summary=summary,
    )
    summary_path = OUT / 'gate2_circular_digital_summary.json'
