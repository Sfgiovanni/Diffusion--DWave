"""g-resolved (here, x-resolved) real-D-Wave-QPU sweep for QuDDPM's "circular states"
family (`N=1`), the companion to `qstate_gate2_tfim_phase_diagram_qpu.py` for the paper's
state-space diagram figure. `generate_circular_pool` already returns each sample's own
angle `x` (`|psi_x> = cos(x)|0> + sin(x)|1>`, `x ~ U[0,2*pi)`); `balanced_pairs`'s `idx`
indexes back into it, so fidelity can be binned by `x` after the fact exactly as the TFIM
scripts bin by `g`. Unlike TFIM, there is no physical phase transition here -- `docs/
QSTATE_DIFFUSION.md` is explicit that this family is purely geometric -- so this checks
something different: whether reconstruction fidelity is uniform around the Bloch circle, or
whether the encoding/reservoir's own basis choice breaks that geometric symmetry.

Same scale as every other real-QPU script in this document (`n_train=n_val=100,n_test=200`,
3 seeds, `num_reads=20`), 12 x-bins (30 degrees each, vs. TFIM's 10 bins over a [0,2] range
of similar total sample count). Reuses the already-cached `n_spins=5` embedding and the
dedicated ledger `qstate_gate2_circular_qpu.py`/`qstate_gate2_correlated_noise_qpu.py`
opened (`dwave_qpu_ledger_qstate_new_targets.json`, ~190.9s remaining going in) --
confirmed with the user beforehand (estimated ~50s of this task's ~85-130s combined
estimate with the correlated-noise sweep).
"""
...
def cell(seed, reservoir):
    ...
    train_pool, _ = targets.generate_circular_pool(N_TRAIN, seed=ds)
    val_pool, _ = targets.generate_circular_pool(N_VAL, seed=ds + 1)
    test_pool, x_test_vals = targets.generate_circular_pool(N_TEST, seed=ds + 2)
    ...
    params = AnnealParams(encoding=encoding, n_spins=5, draw=seed, backend='dwave-qpu',
    ...
    bin_idx = np.digitize(x_test, BIN_EDGES[1:-1])
    rows = []
    for name, pred in (('ridge_classical', pred_c), ('ridge_qrc_qpu', pred_q)):
        ...
        for b in range(N_BINS):
            ...
            if n_in_bin == 0:
                continue
            rows.append(dict(N=N, T=T, data_seed=seed, method=name, x_bin=b,
                             x_bin_center=float((BIN_EDGES[b] + BIN_EDGES[b + 1]) / 2),
                             x_bin_low=float(BIN_EDGES[b]), x_bin_high=float(BIN_EDGES[b + 1]),
...
def main():
    ...
    path = OUT / 'gate2_circular_diagram_qpu.parquet'
    ...
    print('orcamento (compartilhado com circular_qpu.py/correlated_noise_qpu.py):',
         budget.summary(), flush=True)
    reservoir = QpuReservoir(n_spins=5, budget=budget, fast_anneal=False, max_tiles=2,
                             label='qstate-circular-diagram-qpu')
    ...
    payload = dict(
        ...
        scope_note=("Real D-Wave QPU, x-resolved (angle on the Bloch circle) circular-states "
                    "check, N=1 -- the real-hardware companion to the TFIM g-resolved sweep, "
    ...
    summary_path = OUT / 'gate2_circular_diagram_qpu_summary.json'
