...
`qstate_gate2_circular_diagram_qpu.py`, for the same circular-states state-space diagram
figure. Matched settings (n_train/val/test, seeds, `num_reads=20`) to the real-QPU sweep --
same rationale as the TFIM figure's matched simulator/QPU overlay.
"""
...
ROOT = 20260918 + 9000  # same root family as qstate_gate2_circular_diagram_qpu.py
N = 1
T = 200
NOISE_ENSEMBLE = 'hs'
...
NUM_READS = 20  # matched to qstate_gate2_circular_diagram_qpu.py
...
def main():
    ...
    path = OUT / 'gate2_circular_diagram_sqa.parquet'
    ...
    payload = dict(
        provenance=provenance.provenance(
            script='experiments/qstate_gate2_circular_diagram_sqa.py',
        ...
        scope_note=("Free dwave-sqa simulator, x-resolved circular-states check, N=1 -- "
                    "matched settings to qstate_gate2_circular_diagram_qpu.py's real-hardware "
                    "sweep, for a same-figure simulator-vs-QPU overlay."),
        n_rows=len(frame),
    )
    summary_path = OUT / 'gate2_circular_diagram_sqa_summary.json'
